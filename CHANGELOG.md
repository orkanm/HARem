...
### Added
- **Demo Mode Settings Selection**: Refactored Demo Mode in the Settings menu to function consistently with other editable settings. Clicking selects the item (`Demo: [ON]` / `Demo: [OFF]`), dial rotation toggles the state, and clicking again confirms the selection.

### Fixed
- **Hotspot Overlay Input Protection (Issue #42)**: Added input guards for rotary rotation and encoder button presses when the Wi-Fi AP active (`wifi_component.is_ap_active()`) or OTA active (`ota_active`) overlay screen is displayed, preventing background settings or menu modifications. Active settings modes (`settings_mode`, `edit_mode`, `guest_pin_mode`, `control_mode`) are automatically reset to `false` upon entering hotspot mode.
- **Bulk Room Power Control & Click Confirmation**: Removed restrictive local dimmable `st.find("*")` check on ESP32 so Press & Twist events (`rotate_cw_hold` / `rotate_ccw_hold`) are forwarded to Home Assistant on both room items and dimmable items. Updated short-click handler in control mode to execute `send_click_event` when a confirmation prompt (`Turn ON Room?` / `Turn OFF Room?`) is active, enabling Home Assistant to execute the bulk room power command.
